<div>
                ...
                @if(!$user->hasRole('super-admin') && $user->id != auth()->user()->id)
                    ...
                    <div class="grid grid-cols-12 mt-1 mb-1 space-x-0.5"> 
                        <div class="col-span-1 borderC">
                            <p class='flex items-center justify-center h-8'>{{$user->id}}</p>
                        </div>
                        <div class="col-span-5 borderC">
                            <p class='flex items-center justify-center h-8'>{{$user->fullName()}}</p>
                        </div>
                        <div class="col-span-3 borderC">
                            <p class='flex items-center justify-center h-8'>{{$user->email}} </p>
                        </div>
                        <div class="col-span-3">
                            <div class="flex flex-row h-full space-x-0.5">
                                <div class="w-full col-span-3 borderC">
                                    <p class='flex items-center justify-center h-8'>{{$user->blocked ? 'Заблокирован' : 'Активен'}}</p> 
                                </div>
                                <div class="space-x-0.5 flex">
                                    @can('сотрудники (удаление)')
                                        @if ($user->blocked)
                                            <button wire:key='unblock_users_{{$user->id}}' wire:click="unblock({{$user->id}})" class='flex items-center justify-center w-10 colorR'>
                                                <img src="{{asset('/icons/block.svg')}}" class='w-8 h-8 bg-cover fill-white'>
                                            </button>
                                        @else
                                            <button wire:key='block_users_{{$user->id}}' wire:click="block({{$user->id}})" class='flex items-center justify-center w-10 bg-gray-500 hover:bg-gray-400'>
                                                <img src="{{asset('/icons/block.svg')}}" class='w-8 h-8 bg-cover fill-white'>
                                            </button>
                                        @endif
                                    @endcan
    ...
    <livewire:users.edit-employees>
    <livewire:users.create-employees>
    <livewire:users.read-employees>
            ...
            <div class="fixed top-0 left-0 flex items-center justify-center w-full h-full overflow-y-auto bg-gray-600 bg-opacity-50 insert-0">
                <div class="relative m-4 w-2/5 min-w-[40%] max-w-[40%] rounded-lg bg-white font-sans text-base font-light leading-relaxed text-blue-gray-500 antialiased shadow-2xl">
                    ...
                    <div class="relative p-4 font-sans text-base antialiased font-light leading-relaxed border-t border-b border-t-blue-gray-100 border-b-blue-gray-100 text-blue-gray-500">
                        Вы точно хотите удалить сотрудника? Все, связанные с ним действия будут перезаписаны на имя админисратора! Это действие нельзя будет отменить!
                    </div>
