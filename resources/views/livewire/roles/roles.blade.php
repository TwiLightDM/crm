        ...
        @can('роли (создание)')
            <div class="grid grid-cols-7 mb-5">
                <div class="col-span-1 colorA2">
                    <button wire:click='createRole()' class="flex items-center justify-between w-full btn">
                        <img src="{{asset('/icons/add.svg')}}" class='w-6 h-6 ml-1 bg-cover fill-white'>
                        <span class='mr-3'>Роль</span>
                    </button>
                </div>
            </div>
        @endcan
                        ...
                        <div class="col-span-4">
                            <div class="flex flex-row h-full space-x-0.5">
                                ...
                                <div class="space-x-0.5 flex">
                                    <button wire:key='read_roles_{{$role->id}}' wire:click="read({{$role->id}})" class='flex items-center justify-center w-10 colorA'>
                                        <img src="{{asset('/icons/info.svg')}}" class='w-8 h-8 bg-cover fill-white'>
                                    </button>
                                    @can('роли (редактирование)')
                                        <button wire:key='edit_roles_{{$role->id}}' wire:click="edit({{$role->id}})" class='flex items-center justify-center w-10 colorB'>
                                            <img src="{{asset('/icons/edit.svg')}}" class='w-8 h-8 bg-cover fill-white'>
                                        </button>
                                    @endcan
                                    @can('роли (удаление)')
                                        <button wire:key='del_roles_{{$role->id}}' wire:click="try2Delete({{$role->id}})" class='flex items-center justify-center w-10 colorR'>
