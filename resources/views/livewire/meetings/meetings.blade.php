<div>
        ...
        <div class="grid grid-cols-2 mb-5">
                ...
                <div class="grid grid-cols-6">
                    <div class='flex items-center justify-between w-full col-span-3'>
                        ...
                        <select wire:model.live="form.field" class="flex items-center justify-center w-full border-r-0 input">
                            ...
                            <option value="lead_id">Id лида</option>
                            <option value="user_id">Id сотрудника</option>
            ...
            <div>
                <div class="grid grid-cols-12 mt-1 mb-1 space-x-0.5">
                    ...
                    <p class="col-span-2 h-7 borderA text-center colorA2">Сотрудник</p>
                    <p class="col-span-2 h-7 borderA text-center colorA2">Лид</p>
                    <p class="col-span-3 h-7 borderA text-center colorA2">Дата</p>
                ...
                @foreach($meetings as $meeting)
                    <div class="grid h-8 grid-cols-12 mt-1 mb-1 space-x-0.5">
                        <div class="col-span-1 borderC">
                            <p class='flex items-center justify-center h-8'>{{$meeting->id}}</p>
                        </div>
                        <div class="col-span-2 borderC">
                            <p class='flex items-center justify-center h-8'>({{$meeting->user_id}}) {{$meeting->user->fio()}}</p>
                        </div>
                        <div class="col-span-2 borderC">
                            <p class='flex items-center justify-center h-8'>({{$meeting->lead_id}}) {{$meeting->lead->fio()}}</p>
                        </div>
                        <div class="col-span-3 borderC">
                            <p class='flex items-center justify-center h-8'>{{$meeting->date}} </p>
                        </div>
                        <div class="col-span-4">
                            <div class="flex flex-row h-full space-x-0.5">
                                <div class="w-full col-span-3 borderC">
                                    <p class='flex items-center justify-center h-8'>{{$meeting->status}} </p>
                                </div>

                                <div class="space-x-0.5 flex">
                                    <button wire:key='read_meetings_{{$meeting->id}}' wire:click="read({{$meeting->id}})" class='flex items-center justify-center w-10'>
                                        <img src="{{asset('/icons/info.svg')}}" class='w-8 h-8 bg-cover fill-white'>
                                    </button>
                                    @can('встречи (редактирование)')
                                        <button wire:key='edit_meetings_{{$meeting->id}}' wire:click="edit({{$meeting->id}})" class='flex items-center justify-center w-10'>
                                            <img src="{{asset('/icons/edit.svg')}}" class='w-8 h-8 bg-cover fill-white'>
                                        </button>
                                    @endcan
                                    @can('встречи (удаление)')
                                        <button wire:key='del_meetings_{{$meeting->id}}' wire:click="try2Delete({{$meeting->id}})" class='flex items-center justify-center w-10'>
    ...
    <livewire:meetings.edit-meetings>
    <livewire:meetings.read-meetings>
            ...
            <div class="fixed top-0 left-0 flex items-center justify-center w-full h-full overflow-y-auto bg-gray-600 bg-opacity-50 insert-0">
                <div class="relative m-4 w-2/5 min-w-[40%] max-w-[40%] rounded-lg bg-white font-sans text-base font-light leading-relaxed text-blue-gray-500 antialiased shadow-2xl">
                    ...
                    <div class="relative p-4 font-sans text-base antialiased font-light leading-relaxed border-t border-b border-t-blue-gray-100 border-b-blue-gray-100 text-blue-gray-500">
                        Вы точно хотите удалить встречу? Это действие нельзя будет отменить!
                    </div>
